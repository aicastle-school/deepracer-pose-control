...
@DEVICE_INFO_API_BLUEPRINT.route("/restart", methods=["POST"])
def restart_deepracer_core():
    """API to restart the deepracer-core service.
    
    Returns response immediately, then restarts the service in background.

    Returns:
        dict: Execution status if the API call was successful.
    """
    webserver_node = webserver_publisher_node.get_webserver_node()
    webserver_node.get_logger().info("Restart request received")
    
    def do_restart():
        """Background function to restart the service after response is sent."""
        import time
        time.sleep(1)  # Wait 1 second to ensure response is sent
        try:
            webserver_node.get_logger().info("Executing restart command")
            subprocess.run(
            ...
            webserver_node.get_logger().error(f"Restart failed in background: {ex}")
    
    # Start restart in background thread
    restart_thread = threading.Thread(target=do_restart, daemon=True)
    restart_thread.start()
    
    return jsonify(success=True, message="deepracer-core restart initiated")
